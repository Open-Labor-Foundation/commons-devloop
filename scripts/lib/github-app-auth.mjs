 ...
 * github-app-auth.mjs
 *
 * Lets commons-devloop authenticate to GitHub as a dedicated GitHub App
 * installation instead of a personal access token, so commits/PRs/reviews/
 * merges are attributed to the app, not whoever's PAT happened to be in
 * GH_TOKEN. Installation access tokens expire after 1 hour, so this mints
 * one, caches it, and re-mints only once the cached token is close to
 * expiring — then sets it as GH_TOKEN/GITHUB_TOKEN so every existing `gh`/
 * `git` call site (scripts/lib/github.mjs, runtime.mjs) picks it up for free
 * through normal environment inheritance, no call-site changes required.
 *
 * No-ops (returns false) when the three GITHUB_APP_* env vars aren't set, so
 * repos/stacks that still use a plain GH_TOKEN PAT keep working unchanged.
 */
...
/** Resets the in-memory token cache. Test-only. */
export function resetGithubAppAuthCacheForTests() {
  cachedToken = null;
  cachedExpiresAtMs = 0;
}
