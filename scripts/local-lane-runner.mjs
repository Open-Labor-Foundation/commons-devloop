...
// Build the sibling-source-pattern search suggestion from this repo's own
// configured catalog.overlay_root/source_pattern_filenames (AE_CATALOG_OVERLAY_ROOT
// / AE_CATALOG_SOURCE_PATTERN_FILENAMES) plus the record's own related-file
// prefix, rather than a hardcoded path or file convention. A prior version
// hardcoded "agents/catalog/industry-overlays/**" and the research-summary.json/
// manifest.yaml convention here -- a layout/convention no repo on
...
//
// Always includes a direct spec-filename search (AE_CATALOG_SPEC_FILENAME) in
// addition to any configured source_pattern_filenames: some catalog formats
// (labor-commons' README explicitly forbids research-summary.json/manifest.yaml)
// only ever have the catalog entry file itself carrying real, already-vetted
// authority_sources. Suggesting only source_pattern_filenames means this
// always resolves to nothing for such a repo even when real sources exist.
function buildSourcePatternSearches(relatedFilePrefix) {
  ...
  const patternNames = String(env("AE_CATALOG_SOURCE_PATTERN_FILENAMES", ""))
    ...
    .map((name) => name.trim())
    .filter(Boolean);
  const searches = patternNames.flatMap((name) => [
    { query: "authority_sources", glob: `${scope}/**/${name}` },
    { query: "source_audit", glob: `${scope}/**/${name}` }
  ]);
  const specFilename = String(env("AE_CATALOG_SPEC_FILENAME", "")).trim();
  if (specFilename) {
    searches.push({ query: "authority_sources", glob: `${scope}/**/${specFilename}` });
  }
  return searches;
}
